...
int main(int argc, char **argv) {
  ...
  try {
    ...
    backgroundStrength = result["background_strength"].as<int>();
    spikeDurationMs = result["spike_duration_ms"].as<int>();
    spikeDelayMs = result["spike_delay_ms"].as<int>();
  ...
  for (IDirectInputDevice8 *device : forceFeedbackDevices) {
    ...
    if (!di_common::BuildEffectParameters("sine", backgroundEffect,
                                          backgroundParams, backgroundParamSize,
    ...
    if (!di_common::BuildEffectParameters("constant", spikeEffect, spikeParams,
      ...
      std::cerr << "Failed to build constant-force spike effect." << std::endl;
      device->Release();
      directInput->Release();
      return 1;
    }
